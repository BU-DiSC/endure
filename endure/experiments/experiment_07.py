...
class Experiment07(object):
    ...
    def create_tunings(self, wl_idxs, wl_rhos, expected_wls, db_size, bpe, buffer_min):
    ...
    def create_sessions(self, df, samples, writes=False):
        sessions = []
        session = df[df.z0_s + df.z1_s > 0.8].sample(
    ...
    def run(self):
        ...
        for wl_idx in wl_idxs:
            ...
            sessions[wl_idx] = curr_session = self.create_sessions(
                sample_wls_dist, 5, op_mask[-1]
            )
        ...
        tunings = self.create_tunings(
            wl_idxs, rhos, expected_wls, db_size, bpe, buffer_min
        )
        ...
        for wl_idx, design in zip(wl_idxs, tunings):
            ...
            tables.append(table)
            self.de.export_csv_file(pd.concat(tables), "exp_03_checkpoint.csv")
        df = pd.concat(tables)

        self.logger.info("Exporting data from experiment 03")
        self.de.export_csv_file(df, "exp_03_writes_default_direct_false.csv")
