...
std::string kDBPath = "/Users/ndhuynh/sandbox/rocksdb_compact_files_example";
...
public:
    ...
    CompactionTask *PickCompaction( DB* db, const std::string& cf_name) override
    {
        ...
        for (auto level : cf_meta.levels)
        {
            for (auto file : level.files)
            {
                ...
                spdlog::trace("Adding {} to compaction task", file.name);
                input_file_names.push_back(file.name);
            }
        }

        spdlog::trace("Picked compaction to level {}", options_.num_levels - 1);
    ...
    static void CompactFiles(void* arg)
    {
        ...
        assert(task);
        assert(task->db);
        spdlog::trace("Compacting file to level {} ...", task->output_level);
...
int main() {
    ...
    spdlog::info("Compact file examples, provided by Facebook");
    ...
    {
        db->Put(WriteOptions(), std::to_string(i), std::string(500, 'a' + (i % 26)));
    }
