...
MonkeyFilterPolicy::MonkeyFilterPolicy(double _bits_per_element, int _size_ratio, size_t _levels)
  : default_bpe(_bits_per_element),
    size_ratio(_size_ratio),
    levels(_levels),
    default_policy(NewBloomFilterPolicy(_bits_per_element))
{
...
void MonkeyFilterPolicy::CreateFilter(const Slice *keys, int n, std::string *dst) const
{
  this->default_policy->CreateFilter(keys, n, dst);
}

// Incase block based filter is going on we'll use default filter policy
bool MonkeyFilterPolicy::KeyMayMatch(const Slice &key, const Slice &filter) const
{
  return default_policy->KeyMayMatch(key, filter);
}

FilterBitsBuilder * MonkeyFilterPolicy::GetBuilderWithContext(const FilterBuildingContext& context) const
{
...
FilterBitsBuilder * MonkeyFilterPolicy::GetBuilderFromContext(const FilterBuildingContext& context)
{
  if (context.table_options.filter_policy) {
    return context.table_options.filter_policy->GetBuilderWithContext(context);
  } else {
...
FilterBitsReader * MonkeyFilterPolicy::GetFilterBitsReader(const Slice& contents) const
{
  return this->default_policy->GetFilterBitsReader(contents);
}
